...
> **รอบสอง:** M2 — อัปเกรดการ์ดออเดอร์ใน sheet "ออเดอร์ของฉัน": แถบขั้นตอนสถานะ (ชำระเงิน→ยืนยัน→ถึงมือคุณ→คืนแล้ว), ป้ายเตือนใกล้/เลยกำหนดคืน, กล่อง "วิธีคืนชุด" + ปุ่มทักแชต LINE ตอนชุดอยู่กับลูกค้า · M6 — shop.html: tile "เลือกตามงาน" ลิงก์เข้าร้านพร้อมฟิลเตอร์งาน (`?occasion=`), การ์ดสินค้าลิงก์เข้าหน้าชุดจริง (`?garment=`) แทนเด้งเข้าแชต LINE ทุกใบ · M4 — ตรวจแล้ววันที่ sync เข้าชีตรายละเอียดอยู่แล้ว (prefill + เช็คคิวอัตโนมัติ `app.js:1034`) ไม่ผ่าตัดเพิ่ม
> ยังไม่ได้แก้ (งานใหญ่/ตัดสินใจเชิงธุรกิจ): M3 (กวาด catch เงียบทั้งระบบ), เปลี่ยนชื่อ garment.html (กระทบลิงก์/QR ที่พิมพ์ไปแล้ว — ควรตัดสินใจก่อน)
