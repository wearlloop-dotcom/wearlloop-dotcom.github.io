...
## 11. ทางเลือก orchestration: make.com (no-code) vs เขียนเอง (Edge Functions)

make.com มี integration สำเร็จรูปทั้ง HeyGen และ ElevenLabs — ลาก-วางต่อ pipeline ได้โดยไม่เขียนโค้ด

| | **make.com** | **Edge Functions (repo lloop)** |
|---|---|---|
| ความเร็วในการเริ่ม | ⭐ เร็วมาก (ชั่วโมงเดียวได้ pipeline) | ช้ากว่า (ต้องเขียน+PR+deploy) |
| ค่าใช้จ่ายเพิ่ม | ~$9-16/เดือน (แพ็ก Core ขึ้นไป) | 0 (ใช้ Supabase ที่มีอยู่) |
| ต่อกับ second-brain/brand voice | ยาก (อยู่นอกระบบเรา) | ⭐ ตรง ๆ (อ่าน wiki/DB ได้เลย) |
| cost guard / log ลง video_jobs | ทำได้จำกัด | ⭐ คุมเองเต็มที่ |
...
## 12. TikTok vs YouTube — คนละสูตร อย่าใช้คลิปเดียวกันโพสต์สองที่

| | **YouTube** | **TikTok** |
|---|---|---|
| สไตล์ที่เวิร์ก | เรียบร้อย มีโครง สอน/ให้ข้อมูล | ⚠️ **ดิบ ๆ แบบ UGC** — เนี้ยบเกิน = คนเลื่อนผ่าน |
| ความยาว | ยาวได้ (3-10 นาที) | สั้น < 60 วิ (จุดอ่อน avatar ยิ่งโผล่น้อย) |
| โครงคลิป | intro → เนื้อหา → สรุป + B-roll เยอะ | **hook 2 วินาทีแรกตัดสินทุกอย่าง** แล้วเข้าประเด็นเดียว |
| เฟรม avatar | ครึ่งตัว มีฉาก มี B-roll คั่น | ใกล้ ๆ แบบถือมือถือเซลฟี่ แนวตั้ง |
| สคริปต์ | อธิบายเป็นขั้นตอนได้ | พูดเหมือนเล่าให้เพื่อนฟัง 1 ประเด็น/คลิป ห้ามเหมือนโฆษณา |
| จุดตาย | ลิปซิงค์หลุดในช็อตยาว | สคริปต์กลิ่น corporate — คนดู TikTok จับได้ทันที |

**ข้อดีที่ TikTok ให้ฟรี:** คลิปสั้น <60 วิ ทำให้จุดอ่อน avatar (กะพริบแปลก, ปากหลุด) มีเวลาโผล่น้อย
— รีวิววงการบอกว่า short-form คือสนามที่ AI avatar "รอด" ง่ายที่สุด

**กลยุทธ์ที่วงการใช้ (เหมาะกับ LLOOP มาก):** ใช้ AI avatar **ยิงเทสต์ hook/มุมขาย ถูก ๆ เร็ว ๆ**
หลายเวอร์ชัน → ตัวไหนวิ่ง ค่อยถ่ายเวอร์ชันคนจริง/ลงทุนโปรดักชันกับตัวที่พิสูจน์แล้ว
(ลดต้นทุนเทสต์ครีเอทีฟ ~70-80%)

**นัยต่อระบบ:** `video_jobs` ควรมีฟิลด์ `platform` ('youtube' | 'tiktok') และ prompt template
ต้องแยกคนละชุด — สคริปต์ TikTok เขียนสั้น hook นำ ภาษาเพื่อนคุยกัน · สคริปต์ YouTube เป็นโครงสอน
