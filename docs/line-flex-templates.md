...
### แถบรูปชุดย่อ (garment strip) — ใบสถานะทุกใบที่พูดถึงชุด ต้องมีรูปให้รู้ว่า "ชุดไหน"

ใบขายของ (wishlist เข้า / มาใหม่ / ตะกร้าค้าง / สลับชุดสำรอง) ใช้รูปชุดเต็มเป็น `hero` ด้านบน
ส่วน**ใบสถานะ** (จัดส่ง / เตือนคืน / รับเลขพัสดุ / เลยกำหนด / รับคืน / ค่าปรับ / เปลี่ยนการจอง / ผลซ่อม) ไม่ควรใช้รูปเต็ม (จะดูเหมือนโฆษณา) — ใช้แถบรูปย่อนี้แทน วางไว้**ใต้หัวข้อ แทนที่บรรทัด "ชุด →" ในตาราง**:

```json
{ "type": "box", "layout": "horizontal", "spacing": "md", "margin": "lg", "alignItems": "center", "contents": [
  { "type": "image", "url": "{{PHOTO_URL}}", "size": "64px", "aspectRatio": "1:1", "aspectMode": "cover", "flex": 0 },
  { "type": "box", "layout": "vertical", "contents": [
    { "type": "text", "text": "{{GARMENT_NAME}}", "size": "sm", "weight": "bold", "color": "#1A1A1A", "wrap": true },
    { "type": "text", "text": "{{BRAND}} · ไซส์ {{SIZE}}", "size": "xs", "color": "#8C8B86" } ] }
]}
```
(รูป = `photos[0]` ของชุด รูปเดียวกับหน้าเว็บ · ออเดอร์หลายชุดให้ list แถบซ้อนกันใบละแถว สูงสุด 3 แถว + บรรทัด "และอีก {{N}} ชุด")

### ปุ่ม "อ่านเงื่อนไขการเช่า" — ให้ลูกค้ากดเข้าไปอ่านฉบับเต็ม (รายละเอียดครบ ไม่มีปัญหาตีความ)

ไม่สรุปกฎย่อลงการ์ด (เสี่ยงคลาดเคลื่อน/ไม่ครบ) — ใส่**ปุ่ม link เพิ่มอีก 1 ปุ่ม**พาเข้า `rental-terms.html` ฉบับเต็ม โดยลิงก์เจาะไปหมวดที่เกี่ยวกับ step นั้นเลย (เพิ่ม anchor ในหน้าเงื่อนไขให้แล้ว):

```json
{ "type": "button", "style": "link", "height": "sm", "color": "#8C8B86",
  "action": { "type": "uri", "label": "อ่านเงื่อนไขการเช่า", "uri": "https://liff.line.me/{{LIFF_ID}}/rental-terms.html#<หมวด>" } }
```

| ใบ | ลิงก์เจาะไปหมวด | ครอบคลุมเรื่อง |
|---|---|---|
| #1 จองสำเร็จ · #2 รับยอดแล้ว | `#cancel` | เกณฑ์ยกเลิก/เลื่อน/คืนเงิน + การนับวันเช่า |
| #6 จัดส่งแล้ว · #8 เตือนคืน · #9 รับเลขพัสดุ | `#return` | การนับวันคืน วิธีคืน ข้อห้ามซัก/รีด/ซ่อมเอง |
| #10 เลยกำหนดคืน | `#late` | สูตรค่าปรับรายวัน + เพดาน 7 วัน |
| #11 รับคืน+รีวิว · #12 แจ้งค่าดูแล | `#damage` | เกณฑ์ประเมินความเสียหาย มัดจำ สิทธิโต้แย้ง |
| #14 ยกเลิก/เลื่อน/ต่อเวลา | `#cancel` | เกณฑ์คืนเงินทุกกรณี |
| #7 สลับชุดสำรอง | `#backup` | เงื่อนไขชุดสำรอง |
| #13 คืนเงินแล้ว | `#deposit` | เงื่อนไขมัดจำ |

ตำแหน่ง: เป็น**ปุ่ม link ตัวสุดท้าย** สีเทา `#8C8B86` (จางกว่าปุ่มรองปกติ ไม่แย่งความสนใจจาก CTA หลัก) · anchor ที่มีในหน้าเงื่อนไขแล้ว: `#deposit` `#return` `#late` `#cancel` `#damage` `#backup`
...
- ปุ่ม: **ชำระเงิน / ส่งสลิป** → `pay.html?order={{ORDER_ID}}` · ลิงก์: `ดูออเดอร์ของฉัน` → `index.html?go=orders`
...
- ปุ่ม: **ดูออเดอร์ของฉัน** → `index.html?go=orders`
...
- ปุ่ม: **จ่ายส่วนของฉัน** → `pay.html?order={{GROUP_ORDER_ID}}` · ลิงก์: `ดูสถานะกลุ่ม` → `event.html?event={{EVENT_GROUP_ID}}`

## กลุ่มส่ง–ใช้–คืน

### 6. `order_shipped` — จัดส่งแล้ว *(ใหม่ — ปิด gap ใหญ่)*
- icon: `icon-shipped` · kicker: `จัดส่งแล้ว` · **แถบรูปชุดย่อ** (รูป 64px + ชื่อชุด — แทนบรรทัด "ชุด →" ในตาราง)
- หัวข้อ: `ชุดของคุณออกเดินทางแล้ว` (ชื่อชุดอยู่ในแถบรูปแล้ว)
...
- icon: `icon-return` · kicker (clay): `พรุ่งนี้กำหนดคืน` · **แถบรูปชุดย่อ** (รูป 64px + ชื่อชุด — แทนบรรทัด "ชุด →" ในตาราง)
- หัวข้อ: `อย่าลืมส่งชุดกลับนะคะ`
...
- ปุ่ม: **กรอกเลขพัสดุส่งคืน** → `index.html?go=orders&return={{RENTAL_ID}}` · ลิงก์: `ต่อวันเช่า` → `index.html?go=orders&extend={{RENTAL_ID}}`

### 9. `return_tracking_received` — รับเลขพัสดุแล้ว *(ใหม่ — คู่กับ RPC `return_tracking_submit`)*
- icon: `icon-return-received` · kicker: `รับเลขพัสดุส่งคืนแล้ว` · **แถบรูปชุดย่อ** (รูป 64px + ชื่อชุด — แทนบรรทัด "ชุด →" ในตาราง)
...
- icon: `icon-overdue` · kicker (clay): `เลยกำหนดคืน {{DAYS_LATE}} วัน` · **แถบรูปชุดย่อ** (รูป 64px + ชื่อชุด — แทนบรรทัด "ชุด →" ในตาราง)
- หัวข้อ: `ชุดยังไม่ถึงร้านเลยค่ะ`
...
- ปุ่ม: **กรอกเลขพัสดุส่งคืน** → `index.html?go=orders&return={{RENTAL_ID}}` · ลิงก์: `คุยกับแอดมิน` → `{{OA_CHAT_URL}}`

### 11. `return_received_review` — รับคืนแล้ว ขอรีวิว *(ใหม่)*
- icon: `icon-dress` · kicker: `รับชุดคืนแล้ว · สภาพผ่าน` · **แถบรูปชุดย่อ** (รูป 64px + ชื่อชุด — แทนบรรทัด "ชุด →" ในตาราง)
...
- ปุ่ม: **เขียนรีวิว รับเครดิต** → `review.html?rental_id={{RENTAL_ID}}&garment_id={{GARMENT_ID}}&garment_name={{GARMENT_NAME}}&size={{SIZE}}&date_range={{DATE_RANGE}}` · ลิงก์: `หาชุดงานถัดไป` → `index.html`

### 12. `qc_fee_notice` — พบตำหนิ มีค่าดูแลเพิ่ม *(ใหม่ — จุดเก็บเงินที่เดิมเงียบสนิท)*
- icon: `icon-qc-fee` · kicker (clay): `ผลตรวจสภาพชุด` · **แถบรูปชุดย่อ** (รูป 64px + ชื่อชุด — แทนบรรทัด "ชุด →" ในตาราง)
- หัวข้อ: `{{ISSUE_SUMMARY}}`
- ตาราง: ค่าดูแลเพิ่ม → `฿{{FEE}}` · หักจากมัดจำ → `฿{{DEPOSIT}}`
...
- ปุ่ม: **ดูชุดใหม่เข้าร้าน** → `index.html`

### 14. `booking_changed` — ยกเลิก/เลื่อน/ต่อเวลา สำเร็จ *(ใหม่)*
- icon: `icon-booking` · kicker: `{{CHANGE_TITLE}}สำเร็จ` (ยกเลิก/เลื่อนวัน/ต่อเวลา) · **แถบรูปชุดย่อ** (รูป 64px + ชื่อชุด — แทนบรรทัด "ชุด →" ในตาราง)
- หัวข้อ: `{{CHANGE_TITLE}}เรียบร้อยค่ะ` (ชื่อชุดอยู่ในแถบรูป)
...
- ตาราง: เลือกก่อนใครถึง → `{{HOLD_UNTIL}}`
- ปุ่ม: **จองวันนี้เลย** → `index.html?garment={{GARMENT_CODE}}&date={{DATE}}` · ลิงก์: `ออกจากคิว` → `wishlist.html`
...
- ปุ่ม: **จองต่อให้จบ** → `index.html?garment={{GARMENT_CODE}}&date={{START_DATE}}`
...
- ปุ่ม: **ใช้สิทธิ์วันเกิด** → `index.html`
...
- ปุ่ม: **ใช้เครดิตเลือกชุด** → `index.html` · ลิงก์: `แชร์โค้ดต่อ` → `index.html?go=profile`
...
- ปุ่ม: **ดูผลวิเคราะห์** → `index.html?go=stylist` · ลิงก์: `ชุดตรงโทนของฉัน` → `index.html`
...
- ปุ่ม: **ดูนัดของฉัน** → `index.html?go=stylist` · ลิงก์: `เลื่อน / ยกเลิกนัด` → `index.html?go=stylist`

## กลุ่มคอมมูนิตี้–กลุ่ม–สัญญา

### 25. `look_approved` — ลุคขึ้นฟีด *(มีในระบบแล้ว — เพิ่มปุ่ม + ใส่รูปลุค)*
คงข้อความเดิม (kicker `ลุคของคุณขึ้นฟีดแล้ว` · หัวข้อ `ขอบคุณที่แชร์ลุค`) + เพิ่ม hero รูปลุคของลูกค้า และปุ่ม: **ดูลุคในฟีด** → `looks.html` · ลิงก์: `แชร์ต่อ` → `looks.html`
...
- ปุ่ม: **จัดธีมกลุ่ม** → `family.html`

### 27. `repair_quote` — ผลประเมินซ่อม *(ใหม่ — เดิมแจ้งแค่เจ้าของ)*
- icon: `icon-repair` · kicker: `ผลประเมินการซ่อม` · **แถบรูปชุดย่อ** (รูป 64px + ชื่อชุด — แทนบรรทัด "ชุด →" ในตาราง)
- หัวข้อ: `ชุด "{{GARMENT_NAME}}"`
- ตาราง: ผลประเมิน → `{{ASSESSMENT}}` · ค่าซ่อม → `฿{{COST}}` · ระยะเวลา → `{{DAYS}} วัน`
- ปุ่ม: **ยืนยันให้ซ่อม** → `{{OA_CHAT_URL}}` · ลิงก์: `คุยกับแอดมินก่อน` → `{{OA_CHAT_URL}}`
...
5. **ลิงก์ปุ่มผ่านการตรวจ E2E แล้ว** — ทุก URL ชี้พารามิเตอร์ที่หน้าเว็บอ่านจริง: router ของ index รองรับ `?go=orders|profile|stylist|cart|membership|verify`, `?garment=CODE&date=`, `?look=`, `?ref=` และเพิ่มใหม่ `?go=orders&return=RENTAL_ID` / `&extend=RENTAL_ID` (เด้งกรอกเลขพัสดุ/ต่อเวลาให้ทันที) · `review.html` ใช้ `rental_id` · `event.html` ใช้ `event` · `family.html` เข้าด้วย `?join=token` เท่านั้น · deep link รายลุค/วันเกิด/โทนสี/นัดรายตัว ยังไม่มีตัวรับ — ใช้ทางเข้าใกล้สุดไปก่อน ถ้าอยากได้เจาะลึกต้องเพิ่ม router ฝั่งเว็บ
6. ปุ่ม "ยืนยันให้ซ่อม" ชี้เข้าแชท OA (repair.html เป็นหน้า ops ล็อกสิทธิ์พนักงาน ลูกค้าเข้าไม่ได้) — แอดมินยืนยันในแชตแล้วไปกดในระบบแทน
7. ทุก push ควร insert ลง notification inbox ในแอปคู่กัน (kinds ใน `app.js:3676-3698`)
